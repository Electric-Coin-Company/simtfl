...
class BCTransaction:
    ...
    def __init__(self, inputs, output_values, fee, issuance=0):
        ...
        For a coinbase transaction, pass `fee` as a negative value of magnitude equal
    ...
    def is_coinbase(self):
        """Returns `True` if this is a coinbase transaction (it has positive issuance)."""
        return self.issuance > 0
...
class BCBlock:
    def __init__(self, parent, added_score, transactions, is_valid=True):
        """
        Constructs a `BCBlock` with the given parent block, score relative to the parent, and transactions.
        The block need not be valid (since an adversary could provide an invalid block).
